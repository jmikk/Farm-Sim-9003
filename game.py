...
def plant(seed_count):
    ...
    for each in seed_count:
        if id_to_item(each, flip=True):
            new_seed_count = new_seed_count + each.replace(
                each, "[" + id_to_item(each, flip=True) + "]" + each
            )
        else:
...
def plant_crop(crop):
    ...
    if not found:
        cow(
            "Could't find that seed try somthing you own, If I could grow anything I'd grow gold."
        )
        return
    else:
        cow(
    ...
    # ['1. 🥔 Seeds'],
    stats["seed_bag"] = re.sub(
        f"\['{crop}\. . Seeds'\],", "", stats["seed_bag"], count=1
    )
    stats["feild"] = feild
...
def level_up():
    cow(
        "Hooray you leveled up! All of your stats have    gone up by one.  Watch out as you gain more        powerful so do your foes."
    )
    ...
    stats["feild"] = stats["feild"] + "##|" * (
        (1 + int(stats["feildsize"])) ** 2 - int(stats["feildsize"]) ** 2
    )
    # feildsize +1
    ...
    while True:
        ...
        match pick:
            case "1":
                ...
                if MobHp <= 0:
                    cow(
                        "Well done you defeated them unwelcome guest.  You really should invest in a better lock."
                    )
...
"""
    ...
    lost = int(int(stats["gold"]) * 0.20)
    stats["gold"] = int(int(stats["gold"]) * 0.80)
...
def market():
    ...
    while True:
        pick = input(
            "Welcome to the market! Are you here to [1.] Buy! or [2.] Sell! [3.] Leave "
        )
        match pick.lower():
            case "1" | "1." | "buy" | "buy!":
                buy()
...
\/\    ^    /\/
  ...
  *$$$$$$$$$*
            ...
            print(
                f"I would like to buy one of your {id_to_item(pick)}, please!  I only have {int(priceMod) * int(pick)} Gold to spend.  I hope that is alright!"
            )
            picks = input(
                f"""
            ...
            match picks.lower():
                case "1" | "sell" | "yes":
                    ...
                    input(
                        f"Thanks! Here's the gold you now have {stats['gold']} gold in total"
                    )
                    stats["loot_bag"] = stats["loot_bag"].replace(pick, "", 1)
                case "2" | "pass" | "no":
                    input(
                        "Whatever the farmer next door looks like they will sell them cheaper."
                    )
                    pass
                case "3" | "free" | "give":
                    input(
                        "Thank you kind farmer this will feed my family.  May you have good luck and all your dreams come true"
                    )
                    stats["loot_bag"] = stats["loot_bag"].replace(pick, "", 1)
                case "4" | "leave":
                    return
                case _:
                    input(
                        "I'm pretty sure you are being robbed but your mind is clear and you know what, you found the real treasure inner peace."
                    )
                    stats["loot_bag"] = stats["loot_bag"].replace(pick, "", 1)
